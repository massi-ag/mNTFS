...
pub fn check_volume_health<R: Read + Seek>(
    ntfs: &Ntfs,
    reader: &mut R,
) -> Result<HealthReport> {
    ...
    if let Ok(volume_info) = ntfs.volume_info(reader) {
        ...
        if flags.contains(NtfsVolumeFlags::IS_DIRTY) {
            ...
            report.issues
                .push("Volume journal is dirty".to_string());
        }
    }
    ...
    if let Ok(root) = ntfs.root_directory(reader) {
        if let Ok(index) = root.directory_index(reader) {
                ...
                ntfs::indexes::NtfsFileNameIndex::find(&mut finder, ntfs, reader, "hiberfil.sys")
            {
                if let Ok(file) = entry.to_file(ntfs, reader) {
                    if let Some(Ok(data_item)) = file.data(reader, "") {
                        if let Ok(data_attr) = data_item.to_attribute() {
